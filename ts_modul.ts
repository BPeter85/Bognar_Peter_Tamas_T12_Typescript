...
function feltoltoObjektum(tombElem: string[]): snooker[] {
    var beolvasottAdatok: snooker[] = [];
    for (let i: number = 0; i < tombElem.length; i++) {
        let elemDarabok: string[] = tombElem[i].split(";");
        let objektum: snooker = {
            mezSzam: Number(elemDarabok[0]),
            nev: elemDarabok[1],
            nemzetiseg: elemDarabok[2],
            pontSzam: Number(elemDarabok[3])
        }
...
var snookerAdatok: snooker[] = feltoltoObjektum(snookerInfo);
...
function HosegriadoSzint(nap1: number, nap2: number, nap3: number): number {
    ...
    else if (nap1 >= 27 && nap2 >= 27 && nap3 >= 27) {
        return 3;
    }
    else {
        return 0;
...
function Erettsegi(pontok: number[]): [number, number] {
    ...
    for (let i: number = 0; i < pontok.length; i++) {
        szam = osszPont += pontok[i];
        if (szam < 39) {
            jegy = 1;
        }
        else if (szam >= 40 && szam < 59) {
            jegy = 2;
        }
        else if (szam >= 60 && szam < 79) {
            jegy = 3;
        }
        else if (szam >= 80 && szam < 119) {
            jegy = 4;
        }
        else if (szam >= 120 && szam <= 150) {
            jegy = 5;
        }
    }
...
function LeetKod(vizsgaltSzoveg: string): string {
    var maganhangzok: string[] = ["a", "A"];
    var maganhangzok2: string[] = ["o", "O"];
    var maganhangzok3: string[] = ["i", "I"];
    var maganhangzok4: string[] = ["e", "E"];
    var modositottSzo: string = "";
    var modositottSzo2: string = "";
    var modositottSzo3: string = "";
    var modositottSzo4: string = "";

    for (let i: number = 0; i < vizsgaltSzoveg.length; i++) {
        let csereVizsgalo: boolean = false;
        for (let j: number = 0; j < maganhangzok.length; j++) {
            if (vizsgaltSzoveg[i] == maganhangzok[j]) {
                csereVizsgalo = true;
            }
        }
        if (csereVizsgalo == true) {
            modositottSzo += 4;
        }
        else {
    ...
    for (let i: number = 0; i < modositottSzo.length; i++) {
        let csereVizsgalo: boolean = false;
        for (let j: number = 0; j < maganhangzok2.length; j++) {
            if (vizsgaltSzoveg[i] == maganhangzok2[j]) {
                csereVizsgalo = true;
            }
        }
        if (csereVizsgalo == true) {
            ...
            modositottSzo2 += modositottSzo[i];
        }
    }

    for (let i: number = 0; i < modositottSzo2.length; i++) {
        let csereVizsgalo: boolean = false;
        for (let j: number = 0; j < maganhangzok3.length; j++) {
            if (vizsgaltSzoveg[i] == maganhangzok3[j]) {
                csereVizsgalo = true;
            }
        }
        if (csereVizsgalo == true) {
            ...
            modositottSzo3 += modositottSzo2[i];
        }
    }

    for (let i: number = 0; i < modositottSzo3.length; i++) {
        let csereVizsgalo: boolean = false;
        for (let j: number = 0; j < maganhangzok4.length; j++) {
            if (vizsgaltSzoveg[i] == maganhangzok4[j]) {
                csereVizsgalo = true;
            }
        }
        if (csereVizsgalo == true) {
            ...
            modositottSzo4 += modositottSzo3[i];
        }
    }

    return modositottSzo4;
}

//6.Feladat
function LegtobbNyeremeny(vizsgaltObjektum: snooker[]): number {
    let legnyagyobbNyeremeny: number = 0;
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        if (vizsgaltObjektum[i].pontSzam > legnyagyobbNyeremeny) {
            legnyagyobbNyeremeny = vizsgaltObjektum[i].pontSzam;
        }
    }
    return legnyagyobbNyeremeny;
}
